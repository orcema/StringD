...
StringD::StringD(char* StringToParase, const char* Delimiter){
	...
	#ifdef Debug_StringD
	ThisDebug.printf("C:length of StS  = %d with content %s\r\n",StS.length(),StS.c_str());
	#endif
	//m_pos_del=NULL;
	Process();
}
...
StringD::StringD(void* StringToParase, const char* Delimiter,int length){
	memset((char*)StringToParase+length,0,1); // set the null character at the end og the length
...
void StringD::Process(){
	#ifdef Debug_StringD
	ThisDebug.printf("[%s]C:length of StS  = %d with content %s\r\n",__func__,StS.length(),StS.c_str());
	#endif
	...
	#ifdef Debug_StringD
		ThisDebug.printf("C:Number of m_nodes %d\r\n",m_nodes);
		int i;
		for (i=1;i<(m_nodes+1);i++){
			ThisDebug.printf("C:Node %d with content %s\r\n",i, get_node(i));
		}
	#endif
}
...
uint8_t StringD::max_node(){
	#ifdef Debug_StringD
		ThisDebug.printf("C:Number of m_nodes in class function getnodes %d\r\n",m_nodes);
	#endif
	return m_nodes;
}

const char* StringD::get_node(uint8_t node){
	...
	if (node==m_nodes){
		...
		NodeSelected= StS.substring(left,StS.length());

	}else{
//		return (char*)StS.substring(m_pos_del[node-1]+1,m_pos_del[node]-1).c_str();
		NodeSelected=StS.substring(left,right);

	}
	return NodeSelected.c_str();
}
